/**
 * 优化的一致性场景生成脚本
 * 使用真实图生图功能确保BeforeAfterSlider完美对齐
 * 优化参数平衡速度和质量
 */
...
const scenarios = [
  {
    ...
    afterPrompt: 'Same exact kitchen, same lighting, same camera angle and distance, same wooden island - keep only grandfather in white shirt and grandmother in light sweater embracing in center, naturally remove father, mother and young boy while maintaining the same warm poses and composition, sweet elderly couple portrait',
    intensity: 2,
    useExisting: true  // 使用已生成的before图
  },
  {
    key: 'ecommerce', 
    name: '电商场景 - 产品背景替换',
    beforePrompt: 'Professional product photography: white athletic sneakers positioned at exactly 45-degree angle facing camera-right, sitting centered on clean white surface, neutral light gray studio background, commercial lighting setup, high resolution product shot, 800x500 aspect ratio',
    afterPrompt: 'Exact same white athletic sneakers in identical 45-degree position and angle, same camera distance and framing, but transform background to futuristic cyberpunk environment with glowing blue and purple neon circles, dramatic sci-fi lighting effects while keeping product completely unchanged',
    intensity: 3
  },
  {
    key: 'fashion',
...
async function generateScenarioPair(scenario, outputDir) {
  ...
  try {
    ...
    // 检查是否使用已有的before图
    if (scenario.useExisting) {
      const existingPath = path.join(outputDir, `${scenario.key}-before.webp`)
      if (fs.existsSync(existingPath)) {
        console.log(`🔄 使用已存在的Before图片: ${existingPath}`)
        // 读取已有图片并上传到临时URL（实际项目中可能需要不同处理）
        // 这里我们重新生成以确保有URL
        console.log(`📸 重新生成Before图片获取URL...`)
        beforeUrl = await generateTextToImage(scenario.beforePrompt, `${scenario.key}-before.webp`)
      } else {
        beforeUrl = await generateTextToImage(scenario.beforePrompt, `${scenario.key}-before.webp`)
      }
    } else {
...
// 生成所有场景 - 优化版本
async function generateAllOptimizedScenarios() {
  console.log('🚀 开始生成BeforeAfterSlider专用的一致性场景图片...')
  console.log('🎯 使用优化的图生图模式确保基本元素完全一致')
  console.log('⚡ 参数优化：800x500分辨率 + intensity 2-3 + professional风格')
  ...
  console.log(`\n📁 输出目录: ${outputDir}`)
  console.log('🎭 这些图片专为BeforeAfterSlider设计，确保拖动条效果完美！')
...
if (require.main === module) {
  generateAllOptimizedScenarios().catch(console.error)
}

module.exports = { generateAllOptimizedScenarios, scenarios }